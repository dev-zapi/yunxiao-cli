...
**执行任何云效命令前，必须先检查项目根目录是否存在 `YUNXIAO.md` 文件。**

这个文件包含预定义的项目信息，可以避免重复查询 ID，显著提升效率。如果文件存在，优先使用其中的配置；如果不存在，按照常规流程查询。
...
4. **可选字段可能过期**：文件中标注为"可选"的字段（如工作项类型 ID）可能因云效配置变更而失效，使用时注意验证

**首次使用**：如项目没有 YUNXIAO.md，可使用 `yunxiao-setup` 技能交互式创建。
